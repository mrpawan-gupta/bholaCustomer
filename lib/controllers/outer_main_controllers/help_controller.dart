...
class HelpController extends GetxController
  ...
  final PagingController<int, Bookings> pagingControllerServicesLive =
      PagingController<int, Bookings>(firstPageKey: 1);

  final PagingController<int, Bookings> pagingControllerCategoriesLive =
      PagingController<int, Bookings>(firstPageKey: 1);
  ...
  void onInit() {
    ...
    pagingControllerServicesLive.addPageRequestListener(_fetchPageServicesLive);

    pagingControllerCategoriesLive
        .addPageRequestListener(_fetchPageCategoriesLive);
  }
  ...
  void onClose() {
    ...
    pagingControllerServicesLive
      ..removePageRequestListener(_fetchPageServicesLive)
      ..dispose();

    pagingControllerCategoriesLive
      ..removePageRequestListener(_fetchPageCategoriesLive)
      ..dispose();
  ...
  void onTabChange() {
    ...
    tabController.index == 0
        ? pagingControllerServicesLive.refresh()
        : pagingControllerCategoriesLive.refresh();
  ...
  Future<void> _fetchPageServicesLive(int pageKey) async {
    try {
      final List<Bookings> newItems = await _apiCallServicesLive(pageKey);
      final bool isLastPage = newItems.length < pageSize;
      isLastPage
          ? pagingControllerServicesLive.appendLastPage(newItems)
          : pagingControllerServicesLive.appendPage(newItems, pageKey + 1);
    } on Exception catch (error, stackTrace) {
      AppLogger().error(
        message: "Exception caught",
        error: error,
      ...
      pagingControllerServicesLive.error = error;
    } finally {}
    return Future<void>.value();
  }

  Future<void> _fetchPageCategoriesLive(int pageKey) async {
    try {
      final List<Bookings> newItems = await _apiCallCategoriesLive(pageKey);
      final bool isLastPage = newItems.length < pageSize;
      isLastPage
          ? pagingControllerCategoriesLive.appendLastPage(newItems)
          : pagingControllerCategoriesLive.appendPage(newItems, pageKey + 1);
    } on Exception catch (error, stackTrace) {
      AppLogger().error(
        message: "Exception caught",
        error: error,
      ...
      pagingControllerCategoriesLive.error = error;
    } finally {}
    return Future<void>.value();
  }
  ...
  Future<List<Bookings>> _apiCallServicesLive(int pageKey) async {
  ...
  Future<List<Bookings>> _apiCallCategoriesLive(int pageKey) async {
    final Completer<List<Bookings>> completer = Completer<List<Bookings>>();
