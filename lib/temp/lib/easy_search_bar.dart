...
class _EasySearchBarState extends State<EasySearchBar>
  ...
  void updateSyncSuggestions(String input) {
    _suggestions = widget.suggestions!.where((String element) {
  ...
  Widget build(BuildContext context) {
    ...
    final Color searchBackgroundColor = widget.searchBackgroundColor ??
    ...
    final double elevation = widget.elevation ?? appBarTheme.elevation ?? 5;

    final Color cursorColor = widget.searchCursorColor ?? theme.primaryColor;

    final TextStyle searchHintStyle = widget.searchHintStyle ??
        theme.inputDecorationTheme.hintStyle ??
        const TextStyle(color: Colors.grey, fontStyle: FontStyle.italic);

    final IconThemeData searchBackIconTheme = widget.searchBackIconTheme ??
        IconThemeData(size: 24, color: Theme.of(context).primaryColor);

    final IconThemeData searchClearIconTheme = widget.searchClearIconTheme ??
        IconThemeData(size: 24, color: Theme.of(context).primaryColor);

    final SystemUiOverlayStyle systemOverlayStyle = widget.systemOverlayStyle ??
