...
class EcomPaymentScreen extends StatefulWidget {
  const EcomPaymentScreen({super.key});

  @override
  State<EcomPaymentScreen> createState() => _EcomPaymentScreenState();
}

class _EcomPaymentScreenState extends State<EcomPaymentScreen>
    with AfterLayoutMixin<EcomPaymentScreen> {
  final EcomPaymentController controller = Get.find();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: const Text("Ecom Payment Screen"),
            ...
            : condition1 && !condition2
                ? Row(
                    children: <Widget>[
                      ...
                      Expanded(child: backButton()),
                      const SizedBox(width: 16),
                      Expanded(child: viewProductDetailsButton()),
                      const SizedBox(width: 16),
                    ],
                  )
                : !condition1 && !condition2
  ...
  Widget viewProductDetailsButton() {
    return Card(
      margin: EdgeInsets.zero,
      elevation: 4,
      shape: RoundedRectangleBorder(
  ...
  void backDecision() {
    ...
    if (canGoBack) {
      final String route1 = AppRoutes().cartScreen;
  ...
  Future<void> initiatePaymentProcedure() async {
    ...
    response = await controller.orderTransactionAPICall();
    ...
    if (response.$1) {
      ...
      if (result.$1) {
        final bool value = await controller.orderTransactionStatusAPICall();
        value
